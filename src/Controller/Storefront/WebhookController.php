    ...
    private function transactionUpdate(): string
    {
        ...
        if (in_array($this->eventData['transaction']['status'], [ 'PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED' ])) {
            if ($this->eventData['transaction']['status'] === 'DEACTIVATED') {
                ...
                if (in_array($this->eventData['transaction']['payment_type'], ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                    $appendComments = false;
                    $callbackComments = $this->helper->formBasicComments($this->eventData);
                }
                ...
                if (in_array($this->eventData['transaction']['payment_type'], ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE']) && !empty($this->orderReference->getAdditionalDetails())) {
                    $appendComments = false;
                    $this->eventData['transaction']['bank_details'] = $this->helper->unserializeData($this->orderReference->getAdditionalDetails());
                    $callbackComments = $this->helper->formBankDetails($this->eventData);
                } elseif (in_array($this->eventData['transaction']['payment_type'], ['GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                    $appendComments = false;
                    $callbackComments = $this->helper->formBasicComments($this->eventData);
                }
