...
export default class NovalnetSepaPayment extends Plugin {
    ...
    init() {
        ...
        this._createScript(() => {
            document.getElementById(paymentName + '-payment');

            if(paymentName == 'novalnetsepaguarantee' && (config.company === null || !NovalnetUtility.isValidCompanyName(config.company))) {
        ...
        submitButton.addEventListener('click', (event) => {
            ...
            if( sepaId.value !== undefined && sepaId.value !== '' && selectedPaymentId.value === sepaId.value )
            {
				...
				} else if ( paymentName === 'novalnetsepaguarantee' && (config.company === null || !NovalnetUtility.isValidCompanyName(config.company)) )
				{
