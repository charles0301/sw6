...
class NovalnetValidator
{
    ...
    public function isGuaranteeAvailable(SalesChannelContext $salesChannelContext, $transaction, array $settings, string $paymentMethod): string
    {
        ...
        if (empty($settings[$paymentShortName]['minimumOrderAmount'])) {
            $settings[$paymentShortName]['minimumOrderAmount'] = 999;
        }

        if (0 < (int) $settings[$paymentShortName]['minimumOrderAmount'] && (int) $settings[$paymentShortName]['minimumOrderAmount'] > (int) $orderAmount) {
            return 'NO';
        }
        ...
        {
			$count= 0;
			foreach ($settings[$paymentShortName]['cycles'] as $values)
			{
			...
			if( $count == 0  || empty($settings[$paymentShortName]['cycles']))
			{
				return 'NO';
			}
		}

        if (!empty($settings[$paymentShortName]['allowB2B'])) {
